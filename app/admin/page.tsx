    ...
    <div className="min-h-screen bg-[#f0f5f0] p-6">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {CARDS.map((card, i) => (
            <div key={card.key} className="bg-white rounded-2xl shadow-sm border border-[#d8ead8] p-5">
              <div className="flex items-center justify-between mb-4">
                <h2 className="font-semibold text-[#2d6a4f] text-lg">{card.label}</h2>
                <span className="text-xs bg-[#d8f3dc] text-[#2d6a4f] px-2 py-1 rounded-full">#{i + 1}</span>
              </div>
