...
export default function AdminPage() {
  ...
  const filtered = entries.filter((e) => {
    ...
    const matchName = filterName
      ? e.counselor?.toLowerCase().includes(filterName.toLowerCase())
      : true;
    ...
    <div className="min-h-screen bg-[#f0f5f0] p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex gap-4 mb-8 flex-wrap">
          <input
            type="date"
            className="border border-gray-200 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-[#74c69d]"
            value={filterDate}
        ...
        {sortedDates.length === 0 && (
          <p className="text-gray-400 text-center mt-20">No entries found.</p>
        )}

        {sortedDates.map((date) => (
          <div key={date} className="mb-10">
            <h2 className="text-lg font-semibold text-[#40916c] mb-4 border-b border-[#d8ead8] pb-2">
              📅 {date}
            </h2>
            <div className="space-y-6">
              {grouped[date].map((entry) => (
                <div key={entry.counselor + entry.date} className="bg-white rounded-2xl shadow-sm border border-[#d8ead8] p-5">
                  <h3 className="text-base font-bold text-[#2d6a4f] mb-4">
                    👤 {entry.counselor}
                  </h3>
                  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
                    {CARDS.map((card) => (
                      <div key={card.key} className="bg-[#f0f5f0] rounded-xl p-3">
                        <p className="text-xs font-semibold text-[#52b788] uppercase mb-1">
                          {card.label}
                        </p>
                        <p className="text-lg font-bold text-gray-700">
                          {entry[card.key]?.count || 0}
                        </p>
                        {entry[card.key]?.notes && (
